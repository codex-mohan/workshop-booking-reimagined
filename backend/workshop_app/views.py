...
from .models import Profile, User, Workshop, Comment, WorkshopType, AttachmentFile
...
def activate_user(request, key=None):
    ...
    if key is None:
        if (
            ...
            and timezone.now() > user.profile.key_expiry_time
        ):
            status = "1"
            Profile.objects.get(user_id=user.profile.user_id).delete()
            User.objects.get(id=user.profile.user_id).delete()
            return render(request, "workshop_app/activation.html", {"status": status})
...
def user_register(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            send_email(
                request, call_on="Registration", user_position=user_position, key=key
            )
            return render(request, "workshop_app/activation.html")
        else:
            ...
            if form.is_valid():
                ...
                if Workshop.objects.filter(
                    ...
                    messages.add_message(
                        request, messages.SUCCESS, "Workshop proposed successfully"
                    )
                    return redirect(get_landing_page(user))
        # GET request
...
def delete_attachment_file(request, file_id):
    ...
    if file.exists():
        ...
        return redirect(
            reverse("workshop_app:workshop_type_details", args=[file.workshop_type.id])
        )
...
def view_profile(request, user_id):
    ...
    if is_instructor(user) and is_email_checked(user):
        ...
        return render(
            ...
            {"coordinator_profile": coordinator_profile, "Workshops": workshops},
        )
    return redirect(get_landing_page(user))
